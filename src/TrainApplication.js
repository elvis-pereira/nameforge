...
export default class TrainApplication extends Application {
	...
	async saveModel(modelName, model, type) {
		...
		await FilePicker.upload(
			'data',
			'nameforge-models',
			new File([model], `${fileName}.json`, { type: 'application/json' }),
			{},
			{ notify: false }
		);
		await FilePicker.upload(
			'data',
			'nameforge-models',
			new File([JSON.stringify(userModels, null, 2)], 'models.json', { type: 'application/json' })
		);

		game.modules.get('nameforge').models.userModels[fileName] = {
			name: modelName,
			path: `nameforge-models/${fileName}.json`,
			type
		};
	}
	...
	activateListeners(html) {
		...
		form.addEventListener('submit', async (event) => {
			...
			const bestIteration = { error: 1, model: null, number: 0 };
			const nameforge = new NameForge();

			worker.postMessage({ name: 'train', names: trainingData, options });
